...
const int size_board = 2*N, winning_score = 2*N;
...
class Shuneo { 
        public:
                static std::pair<int,int> giveMeAMove(int table[2*N][2*N]) {
                        char Long[2*N][2*N];
                        for (int i = 0; i < 2*N; ++i) {
                            for (int j = 0; j < 2*N; ++j) {
                                if (table[i][j] == SHUNEO) Long[i][j] = player;
                                else if (table[i][j] == GUEST) Long[i][j] = opponent;
                                else Long[i][j] = empty_cell;
                            }
                        }
                        initializeHashTable();
                        Move best_move = best_move_of_player(Long);
                        return std::make_pair(best_move.row, best_move.col);
                        /*
                        srand(time(NULL));
                        int x = 0, y = 0;
                        while (table[x][y] != BLANK) {
                                x = (rand() % (2 * N) + rand() % (2 * N)) % (2 * N);
                                y = (rand() % (2 * N) + rand() % (2 * N)) % (2 * N);
                        }
